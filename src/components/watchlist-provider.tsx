...
interface WatchlistStore {
  ...
  update: (
    id: string,
    fn: (channel: WatchlistChannel) => WatchlistChannel,
...
export function WatchlistProvider({
  ...
  initialChannels: WatchlistChannel[];
}) {
  ...
  const persist = useCallback(async (channel: WatchlistChannel) => {
    ...
    if (!response.ok || !result.channel)
      throw new Error(result.error?.message ?? "Could not save the watchlist.");
    return result.channel;
  }, []);
  const upsert = useCallback(
    async (channel: WatchlistChannel) => {
      const persisted = await persist(channel);
      setLocal([
        persisted,
        ...channelsRef.current.filter(
          (item) => item.id !== channel.id && item.youtubeChannelId !== channel.youtubeChannelId,
        ),
      ]);
      return persisted;
    },
  ...
  const update = useCallback(
    async (id: string, fn: (channel: WatchlistChannel) => WatchlistChannel) => {
  ...
  const remove = useCallback(
    async (id: string) => {
      const response = await fetch(`/api/watchlist?channelId=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
      if (!response.ok) {
    ...
    void (async () => {
      try {
        ...
        for (const legacy of legacyChannels) {
          if (next.some((channel) => channel.youtubeChannelId === legacy.youtubeChannelId))
            continue;
        ...
        console.error(
          "watchlist_local_migration_failed",
  ...
  const ready = true;
  const value = useMemo(
    () => ({ channels, ready, upsert, update, remove }),
    [channels, ready, upsert, update, remove],
  );
