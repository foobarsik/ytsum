...
export function PriorityBadge({ priority }: { priority: Priority }) {
  const map = {
    recommended: ["badge-green", "Recommended", CheckCircle2],
    low: ["badge-gray", "Low priority", Clock3],
    unclear: ["badge-amber", "Unclear", CircleHelp],
  } as const;
  const [className, label, Icon] = map[priority];
  return (
...
export function TranscriptBadge({ status }: { status: TranscriptStatus }) {
  ...
  return (
    <span
      className={`badge ${good ? "badge-green" : status === "failed" ? "badge-red" : "badge-amber"}`}
    >
      {good ? (
