...
export async function POST(request: Request) {
  try {
    ...
    if (error instanceof z.ZodError) {
      console.warn("ai_analysis_invalid_request", { issues: error.issues.map((issue) => issue.code) });
      return NextResponse.json({ error: { code: "INVALID_REQUEST", message: `Invalid analysis request. The batch may exceed the ${process.env.MAX_VIDEOS_PER_BATCH ?? 25}-video limit.`, retryable: false } }, { status: 400 });
    }
    const mapped = mapExternalError(error); console.error("ai_analysis_failed", { code: mapped.code }); return NextResponse.json({ error: { code: mapped.code, message: mapped.message, retryable: mapped.retryable } }, { status: 502 });
  }
}
