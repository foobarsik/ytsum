...
export async function PUT(request: Request) {
  try {
    ...
    if (!userId)
      return NextResponse.json(
        { error: { message: "Sign in to save the watchlist." } },
        { status: 401 },
      );
    ...
    const message =
      error instanceof z.ZodError
        ? "Could not save the watchlist because a date or field has an invalid format."
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!userId)
      return NextResponse.json(
        { error: { message: "Sign in to update the watchlist." } },
        { status: 401 },
      );
    ...
    return NextResponse.json(
      {
        error: {
          message: error instanceof Error ? error.message : "Could not delete the channel.",
        },
      },
      { status: 400 },
    );
