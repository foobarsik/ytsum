...
const importSchema = addPlaylistSchema.extend({ knownVideoIds: z.array(z.string().min(1)).max(5_000).optional(), addedAfter: z.string().datetime().optional(), pageToken: z.string().min(1).optional() });
...
export async function POST(request: Request) {
  try {
    ...
    const playlist = await provider.getPlaylist(id, { excludeVideoIds: input.knownVideoIds ? new Set(input.knownVideoIds) : undefined, addedAfter: input.addedAfter, pageToken: input.pageToken, allowEmpty: Boolean(input.knownVideoIds) });
