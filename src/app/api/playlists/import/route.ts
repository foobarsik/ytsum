...
const importSchema = addPlaylistSchema.extend({ knownVideoIds: z.array(z.string().min(1)).max(500).optional(), addedAfter: z.string().datetime().optional() });
...
export async function POST(request: Request) {
  try {
    ...
    const playlist = await provider.getPlaylist(id, { excludeVideoIds: input.knownVideoIds ? new Set(input.knownVideoIds) : undefined, addedAfter: input.addedAfter, allowEmpty: Boolean(input.knownVideoIds) });
