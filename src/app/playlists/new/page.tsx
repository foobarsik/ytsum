...
export default function AddPlaylistPage() {
  ...
  return <div className="shell py-8 sm:py-12"><Link href="/" className="mb-7 inline-flex items-center gap-2 text-sm font-semibold text-stone-600 hover:text-stone-950"><ArrowLeft size={16}/>Back to playlists</Link><div className="mx-auto max-w-2xl"><div className="mb-7"><p className="eyebrow mb-2">New source</p><h1 className="text-3xl font-bold">Add a playlist</h1><p className="muted mt-2">Import metadata and choose how the playlist should be analyzed.</p></div><div className="mb-5 flex gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-[#15803D]"><CircleCheck className="mt-0.5 shrink-0" size={18}/><p><strong>Live YouTube import enabled.</strong> Metadata and available captions are loaded automatically.</p></div><form onSubmit={submit} className="surface p-5 sm:p-7" noValidate><div className="mb-5"><label className="label" htmlFor="playlist-url">YouTube playlist URL</label><input id="playlist-url" className="field" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://youtube.com/playlist?list=..." aria-invalid={Boolean(error)} required/><p className="muted mt-2 text-xs">Public or unlisted playlists only.</p></div><div className="mb-5"><label className="label" htmlFor="title">Playlist name <span className="font-normal text-stone-500">(optional)</span></label><input id="title" className="field" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={120} placeholder="Use title from YouTube"/></div><fieldset className="mb-5"><legend className="label">Agent mode</legend><div className="grid gap-2 sm:grid-cols-3">{([['inbox','Inbox','What is worth my time?'],['learning','Learning','What order should I follow?'],['research','Research','What holds across sources?']] as const).map(([value,label,help]) => <label key={value} className={`cursor-pointer rounded-lg border p-3 ${mode === value ? 'border-[#EA580C] bg-orange-50' : 'border-stone-200'}`}><input type="radio" name="mode" value={value} checked={mode === value} onChange={() => setMode(value)} className="mr-2 accent-[#EA580C]"/><strong className="text-sm">{label}</strong><span className="mt-1 block pl-5 text-xs text-stone-500">{help}</span></label>)}</div></fieldset><div className="mb-6"><label className="label" htmlFor="depth">Summary depth</label><select id="depth" className="field" value={depth} onChange={(e) => setDepth(e.target.value as SummaryDepth)}><option value="brief">Brief</option><option value="normal">Normal</option><option value="deep">Deep</option></select></div>{error && <p role="alert" className="mb-5 rounded-lg bg-red-50 p-3 text-sm font-medium text-red-800">{error}</p>}<button className="button button-primary w-full" disabled={loading}>{loading ? <><Loader2 className="animate-spin" size={17}/>Importing playlist and captions…</> : "Import playlist"}</button></form></div></div>;
}
