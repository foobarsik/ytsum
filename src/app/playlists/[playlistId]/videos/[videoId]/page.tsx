...
import { ArrowLeft, BookOpen, Captions, ExternalLink, FileText, Loader2, RotateCcw, Save, Trash2 } from "lucide-react";
...
export default function VideoPage() {
  ...
  const params = useParams<{ playlistId: string; videoId: string }>(); const { playlists, ready, update } = usePlaylists(); const [defaultLanguage] = useSummaryLanguage(); const [showTranscript, setShowTranscript] = useState(false); const [showTranscriptReader, setShowTranscriptReader] = useState(false); const [text, setText] = useState(""); const [busy, setBusy] = useState<"summary" | "transcript" | "cleaning" | null>(null); const [notice, setNotice] = useState("");
  ...
  const playlistId = playlist.id; const videoId = video.id; const youtubeVideoId = video.youtubeId; const summaryDepth = playlist.summaryDepth; const currentVideo = video; const summaryLanguage = video.summaryLanguage ?? defaultLanguage; const metadata = [video.channel, video.duration !== "Unknown" ? video.duration : "", formatPublishedDate(video.publishedAt)].filter(Boolean).join(" · "); const hasTranscript = Boolean(video.transcript);
  function patchVideo(fields: Parameters<typeof updateVideo>[2]) { update(playlistId, (p) => updateVideo(p, videoId, fields)); }
  function deleteVideo() {
    if (!window.confirm(`Remove “${currentVideo.title}” from this YouSum playlist? The YouTube playlist will not be changed.`)) return;
    update(playlistId, (current) => removeVideoFromPlaylist(current, videoId));
    router.replace(`/playlists/${playlistId}`);
  }
  function saveTranscript() { if (text.trim().length < 30) { setNotice("Add at least 30 characters of transcript text."); return; } patchVideo({ transcript: text.trim(), cleanedTranscript: undefined, transcriptCleanedAt: undefined, transcriptEditVersion: undefined, transcriptEditLanguage: undefined, transcriptStatus: "manually_added", priority: "unclear", priorityReason: "Transcript added. Run playlist analysis to calculate priority." }); setShowTranscript(false); setShowTranscriptReader(false); setNotice("Transcript saved. It is now available for summary and analysis."); }
  async function loadTranscript() {
    ...
    try {
      ...
      if (result.status === "available" && result.transcript) {
        patchVideo({ transcript: result.transcript, cleanedTranscript: undefined, transcriptCleanedAt: undefined, transcriptEditVersion: undefined, transcriptEditLanguage: undefined, transcriptStatus: "available", priority: "unclear", priorityReason: "Transcript imported. Run playlist analysis to calculate priority." });
  ...
  async function readTranscript(force = false) {
    if (!force && showTranscriptReader) { setShowTranscriptReader(false); return; }
    if (!force && currentVideo.cleanedTranscript && currentVideo.transcriptEditVersion === 3 && currentVideo.transcriptEditLanguage === summaryLanguage) { setShowTranscriptReader(true); return; }
    if (!currentVideo.transcript) return;
    setBusy("cleaning"); setNotice("");
    try {
      const response = await fetch("/api/ai/clean-transcript", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ transcript: currentVideo.transcript, language: summaryLanguage }) });
  ...
  async function summarize() {
    ...
    try {
      const canUseEditedTranscript = currentVideo.transcriptEditVersion === 3 && currentVideo.transcriptEditLanguage === summaryLanguage;
      const videoForSummary = { ...currentVideo, transcript: canUseEditedTranscript ? currentVideo.cleanedTranscript : currentVideo.transcript };
      const response = await fetch("/api/ai/summarize", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ video: videoForSummary, depth: summaryDepth, language: summaryLanguage }) });
