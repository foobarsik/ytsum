...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <html lang="en"><body><PlaylistProvider><WatchlistProvider><PwaClient /><header className="border-b border-[var(--border)] bg-white"><div className="shell flex h-16 items-center justify-between"><Link href="/" className="flex items-center gap-2 font-bold"><span className="grid size-8 place-items-center rounded-lg bg-[var(--accent)] text-white"><BrainCircuit size={18} aria-hidden="true" /></span>PlaylistMind</Link><div className="flex items-center gap-2 sm:gap-3">{!hasYouTubeApi && <span className="badge badge-amber hidden sm:inline-flex">Demo</span>}<Link href="/watchlist" className="button button-secondary"><Eye size={17}/><span className="hidden sm:inline">Watchlist</span></Link><AuthControls/><Link href="/settings" className="button button-secondary" aria-label="Settings"><Settings size={17}/><span className="hidden sm:inline">Settings</span></Link><Link href="/playlists/new" className="button button-primary">Add playlist</Link></div></div></header><main>{children}</main></WatchlistProvider></PlaylistProvider></body></html>;
}
