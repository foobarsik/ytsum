...
type ChannelRow = {
  id: string; youtube_channel_id: string; uploads_playlist_id: string; title: string;
  description: string; thumbnail_url: string | null; topic: string;
  last_checked_at: string | null; created_at: string;
};
type VideoRow = {
  id: string; channel_id: string; youtube_video_id: string; title: string; description: string;
  published_at: string; thumbnail_url: string | null; transcript_status: WatchlistVideo["transcriptStatus"];
};
...
export async function loadWatchlist(supabase: SupabaseClient): Promise<WatchlistChannel[]> {
  const { data: channelData, error: channelError } = await supabase
    ...
    .select("id,youtube_channel_id,uploads_playlist_id,title,description,thumbnail_url,topic,last_checked_at,created_at")
  ...
  const { data: videoData, error: videoError } = await supabase
    ...
    .select("id,channel_id,youtube_video_id,title,description,published_at,thumbnail_url,transcript_status")
  ...
  if (videos.length) {
    const { data, error } = await supabase
      ...
      .in("watchlist_video_id", videos.map((video) => video.id));
    fail(error, "Could not load watchlist insights");
    insights = (data ?? []) as InsightRow[];
  }

  const insightByVideo = new Map(insights.map((insight) => [insight.watchlist_video_id, insight.output]));
...
export async function saveWatchlistChannel(
  ...
  const persistedVideos = channel.videos.length
    ? await supabase
      .from("watchlist_videos")
      .upsert(channel.videos.map((video) => ({
  ...
  const videoIdByYoutubeId = new Map(
    (persistedVideos.data ?? []).map((video: { id: string; youtube_video_id: string }) => [video.youtube_video_id, video.id]),
  );
...
export async function deleteWatchlistChannel(supabase: SupabaseClient, userId: string, channelId: string): Promise<void> {
  const { error } = await supabase.from("watchlist_channels").delete().eq("id", channelId).eq("user_id", userId);
