...
describe("YouTube channel input", () => {
  it("parses handles and channel URLs", () => {
    ...
    expect(parseYouTubeChannelInput("https://youtube.com/@GoogleDevelopers/videos")).toEqual({
      handle: "@GoogleDevelopers",
    });
    expect(
      parseYouTubeChannelInput("https://youtube.com/channel/UC1234567890123456789012"),
    ).toEqual({ id: "UC1234567890123456789012" });
  });
});
...
describe("YouTube channel provider", () => {
  it("loads channel metadata and its uploads playlist", async () => {
    ...
    await expect(new YouTubeChannelProvider("key").getChannel("@channel")).resolves.toMatchObject({
      id: "UC1234567890123456789012",
  ...
  it("loads latest upload metadata", async () => {
      ...
      vi.fn().mockResolvedValue(
        new Response(
          JSON.stringify({
            items: [
              {
                snippet: { title: "Video", description: "Description" },
                contentDetails: {
                  videoId: "abcdefghijk",
    ...
    await expect(new YouTubeChannelProvider("key").getLatestVideos("UU123", 5)).resolves.toEqual([
