import type { AgentMode, PlaylistAnalysis, SummaryDepth, Video, WatchlistInsight, WatchlistTermReview } from "@/domain/types";
...
export interface AIProvider {
  ...
  inspectWatchlistTerms(input: { transcript: string; description: string; language: SummaryLanguage }): Promise<WatchlistTermReview[]>;
  analyzeWatchlistVideo(input: { channelTitle: string; videoTitle: string; topic: string; transcript: string; terms: WatchlistTermReview[]; priorInsights: WatchlistInsight[]; language: SummaryLanguage }): Promise<WatchlistInsight>;
