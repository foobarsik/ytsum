  ...
  async getPlaylist(
    ...
    try {
      ...
      if (!playlistResponse.ok)
        throw new Error(playlistData.error?.message ?? "YouTube request failed");
      const playlist = playlistData.items?.[0]?.snippet;
      if (!playlist)
        throw new AppError("PRIVATE_PLAYLIST", "This playlist is private or cannot be accessed.");
      ...
      do {
        ...
        for (const item of data.items ?? []) {
          ...
          if (
            options.addedAfter &&
            (!snippet?.publishedAt || snippet.publishedAt < options.addedAfter)
          )
            continue;
          videos.push({
            id: youtubeId,
            ...
            priority: "unclear",
            priorityReason: "Transcript is not available yet.",
          });
          if (videos.length >= this.maxVideos) break;
        }
        pageToken = data.nextPageToken;
      } while (pageToken && videos.length < this.maxVideos);
      if (!videos.length && !options.allowEmpty)
        throw new AppError("EMPTY_PLAYLIST", "This playlist has no accessible videos.");
      return {
        id: playlistId,
