...
describe("OpenRouter JSON parsing", () => {
  ...
  it("repairs common malformed JSON", () => {
    expect(parseAIJson("{cleanedText: 'Readable text',}")).toEqual({
      cleanedText: "Readable text",
    });
  });
});
...
describe("Watchlist signal ranking", () => {
  const base = {
    whyItMatters: "It changes the development workflow.",
    whoIsAffected: ["Developers"],
    evidenceFromVideo: "Explicit workflow description.",
    counterpoint: "Still early.",
    productOpportunity: "Agent orchestration",
  ...
  it("keeps decision signals and removes hype and near-duplicates", () => {
    const ranked = rankWatchlistSignals([
      {
        ...base,
        claim: "Developers are moving from direct prompts to agent workflow design.",
        signalType: "workflow_shift",
      },
      {
        ...base,
        claim: "Developer work moves from direct prompting toward designing agent workflows.",
        signalType: "workflow_shift",
      },
  ...
  it("preserves an unverified raw term but blocks its product opportunity", () => {
    const ranked = rankWatchlistSignals(
      [
        {
          ...base,
          claim: "OMD is emerging as an agent registration standard.",
          evidenceFromVideo: "The sponsor calls OMD a standard.",
          signalType: "tooling_opportunity",
        },
      ],
      [
        {
          rawTerm: "OMD",
          termType: "acronym",
          contextExcerpt: "OMD standard",
          asrRisk: "high",
          canonicalTerm: null,
          canonicalConfidence: "low",
          verificationRequired: true,
          status: "needs_verification",
          notes: "No source-backed expansion.",
        },
      ],
    );
