...
describe("YouTube playlist pagination", () => {
  it("returns a cursor after a bounded page without skipping items", async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ items: [{ snippet: { title: "Playlist", description: "" } }] }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ nextPageToken: "page-2", items: [{ snippet: { title: "Video", channelTitle: "Channel", publishedAt: "2026-01-01T00:00:00Z", resourceId: { videoId: "abcdefghijk" }, thumbnails: {} } }] }), { status: 200 }));
