...
describe("YouTube transcript provider", () => {
  ...
  it("keeps per-video failures isolated while enriching a playlist", async () => {
    const provider: TranscriptProvider = {
      getTranscript: async (videoId) => videoId.endsWith("1")
        ? { status: "available", transcript: "A sufficiently complete imported transcript." }
        : { status: "unavailable" },
    };
    const videos = demoPlaylists[0].videos.slice(0, 2).map((video) => ({ ...video, transcript: undefined, transcriptStatus: "unavailable" as const }));
    const result = await enrichVideosWithTranscripts(videos, provider, 2);
    expect(result[0]).toMatchObject({ transcriptStatus: "available", transcript: "A sufficiently complete imported transcript." });
