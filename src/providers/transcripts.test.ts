...
import {
  ...
  ManagedTranscriptProvider,
  transcriptProviderFromEnv,
} from "./transcripts";
...
describe("transcript provider selection", () => {
  ...
  it("uses the managed provider when configured", () => {
    expect(transcriptProviderFromEnv({ TRANSCRIPT_API_KEY: "test-key" })).toBeInstanceOf(
      ManagedTranscriptProvider,
    );
  });
...
describe("Supabase transcript provider", () => {
  it("sends the shared secret only from the server", async () => {
    ...
    await expect(
      new ExternalTranscriptProvider(
        "https://example.test/transcript",
        "shared-secret",
        "ru",
      ).getTranscript("dQw4w9WgXcQ"),
    ).resolves.toEqual({ status: "available", transcript: "Caption text" });
...
describe("managed transcript provider", () => {
  ...
  it("maps a confirmed no-captions response to unavailable", async () => {
    ...
    await expect(
      new ManagedTranscriptProvider("secret").getTranscript("dQw4w9WgXcQ"),
