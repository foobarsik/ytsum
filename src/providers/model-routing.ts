...
export function routeModel(task: ModelTask, config: ModelConfig): string {
  const model =
    task === "triage" ? config.cheap : task === "synthesis" ? config.deep : config.default;
  if (!model) throw new Error(`Model for ${task} is not configured`);
  return model;
}

export function taskForMode(mode: AgentMode): ModelTask {
  return mode === "research" ? "synthesis" : "triage";
}
