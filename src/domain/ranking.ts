...
export function applyRecommendations(
  ...
  return videos.map((video) => {
    ...
    const priority =
      requestedPriority === "low" && video.transcriptStatus === "unavailable"
        ? "unclear"
        : requestedPriority;
    return {
      ...video,
      ...
      priorityReason:
        priority === "unclear"
          ? `Unclear: transcript unavailable. ${decision.reason}`
          : decision.reason,
    };
  });
}
...
export function deduplicatePlaylists(playlists: Playlist[]): Playlist[] {
  ...
  return playlists.filter(
    (playlist) =>
      !seen.has(playlist.youtubePlaylistId) && Boolean(seen.add(playlist.youtubePlaylistId)),
  );
}
