...
Keep the secret/service-role key server-only. The browser auth client uses only the publishable key. Watchlist content is still local to the signed-in browser until the separate Supabase persistence migration is connected to the UI.
